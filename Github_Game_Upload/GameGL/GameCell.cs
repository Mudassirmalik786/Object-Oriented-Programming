...
namespace GameGUI.GameGL
{
    public class GameCell
    {
        ...
        const int width = 30; // cell size 
        const int height = 40; // object size
        public GameCell(int row, int col, GameGrid grid)
        {
            this.row = row;
            this.col = col;
            pictureBox = new PictureBox();
            pictureBox.Left = col * width + (width - height) / 2;
            pictureBox.Top = row * width + (width - height) / 2;
        ...
        public GameCell nextCell(GameDirection direction)
        {
            ...
            if (direction == GameDirection.Left)
            {
                if (this.col > 0)
                {
                    ...
                    if (ncell.CurrentGameObject.GameObjectType != GameObjectType.WALL && ncell.CurrentGameObject.GameObjectType != GameObjectType.OBSTACLE)
                    {
                        return ncell;
                    }
                }
